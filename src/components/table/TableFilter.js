...
export class TableFilter extends React.Component {
    ...
    return this.props.rows.filter((el) => {
      ...
      for (let i = 0; i < this.state.activeFilters.length; i++) {
        ...
        if (propertyParts.length === 1) {
          ...
          element =
            el[propertyParts[0]] && el[propertyParts[0]][propertyParts[1]];
        }
        numberOfFilteredValuesFound +=
          element &&
          element
            .toLowerCase()
            .indexOf(this.state.activeFilters[i].value.toLowerCase()) > -1;
      }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevState.activeFilters.length !== this.state.activeFilters.length ||
  ...
  filterAdded = (field, value) => {
    ...
    let activeFilters = [
      ...
      { label: filterText, property: field.property, value: value },
    ];
