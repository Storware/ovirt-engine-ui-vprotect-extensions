...
const prepareChartDataBackups = (props, state) => {
  ...
  if (!props.datasets.backupsHistory || props.datasets.backupsHistory.length === 0) {
    return state
  }
...
const prepareChartDataRestoreTime = (props, state) => {
  ...
  for (let i = limit - 1 + shiftChart; i >= 0 + shiftChart; i--) {
    if (restores[i]) {
      ...
      const total = importTime + mountTime + queuedImportTime + queuedMountTime + queuedRestoreTime + restoreTime
      if (chartData.max < total) {
        chartData.max = total
      }
    ...
    return (
      <div>
        <div className='d-flex justify-content-end mt-3'>
          <div>
            <Button className='mx-2' label='Daily activity' onClick={() => {
              this.setState({
                ...this.state,
                option: 'days'
              })
            }} />
            <Button className='mx-2' label='Backup size' onClick={() => {
              this.setState({
                ...this.state,
                option: 'backups'
              })
            }} />
            <Button className='mx-2' label='Backup time' onClick={() => {
              this.setState({
                ...this.state,
                option: 'time'
              })
            }} />
            <Button className='mx-2' label='Restore time' onClick={() => {
              this.setState({
                ...this.state,
                option: 'restoreTime'
              })
