...
const prepareChart = {
  'size': (props, state) => {
    ...
    if (maxValue > 1024 * 1024) {
      unit = '\u00A0MiB'
      rate = 1024 * 1024
      stepSize = Math.ceil(maxValue / (baseNumber)) * baseNumber * 1.024 * 1.024 / 5
    }
    if (maxValue > 1024 * 1024 * 1024) {
      unit = '\u00A0GiB'
      rate = 1024 * 1024 * 1024
      stepSize = Math.ceil(maxValue / (baseNumber)) * baseNumber * 1.024 * 1.024 * 1.024 / 5
    }
    if (maxValue > 1024 * 1024 * 1024 * 1024) {
      unit = '\u00A0TiB'
      rate = 1024 * 1024 * 1024 * 1024
      stepSize = Math.ceil(maxValue / (baseNumber)) * baseNumber * 1.024 * 1.024 * 1.024 * 1.024 / 5
    }
  ...
  'time': (props, state) => {
    ...
    if (maxValue > 1000 * 60 * 60) {
      unit = '\u00A0h'
      rate = 1000 * 60 * 60
      stepSize = Math.ceil(maxValue / (baseNumber)) * baseNumber * 0.6 * 0.6 / 2
    }
    ...
    return {
      ...
      options: {
        ...
        tooltips: {
          ...
          callbacks: {
            label: (tooltipItem) => {
              return tooltipItem.yLabel ? ` ${duration(tooltipItem.yLabel)}` : null
            },
            footer: (data) => {
              return `     ${duration(data.map(el => el.yLabel).reduce((a, b) => a + b, 0))}`
            }
          }
        },
        maintainAspectRatio: false,
