...
const Radio = ({value, ...props}) => {
    ...
    useEffect(() => {
        ...
        let optionalValue = shouldSelectFirstOption ? props.options[0].value : value
    ...
    return (
        <div className='pt-3'>
            ...
            {props.options.map(el => {
                return <div className='col d-flex my-2'>
                    <RadioButton
                        value={el.value}
                        inputId={el.name}
                    ...
                    />
                    <label htmlFor={el.name} className="pl-2 p-radiobutton-label">{el.name}</label>
                </div>
            })}
        </div>
    )
