...
import {checkIfIscsiMountable, getBackupFilesystems, getMountedBackup, submitTask} from '../../../../store/mount-backup-modal/actions';
import {
    selectBackupFiles, selectIscsiMountable,
...
const manuallyMountParameterLabel = (el) => {
    return <span>{el.fileSystem?.volume} {el.fileSystem?.label ? '['+el.fileSystem?.label+']':''} ({el.fileSystem?.type}, <Filesize bytes={el.fileSystem?.size}/>)</span>
}

export const nameTemplate = (rowData) => {
    return <span>{rowData.path.split('/')[rowData.path.split('/').length-1]}</span>
}
        ...
        <div>
            <div className='form'>
                ...
                <BackupDropdown value={task.backup}
                                required
                                onChange={(event) => {
                                    autoMountFileSystem.mountPath = '/mnt/vprotect/' + event.protectedEntity.name
                ...
                {task.mode?.name === 'AUTO' &&
                <Text label='Mount point for backup'
                ...
                <div>
                  <label>PARAMETERS FOR MOUNTING FILESYSTEMS MANUALLY</label>
                    {mountedFileSystems['MANUAL'].map(el => {
                        return <div>
                            <Check label={manuallyMountParameterLabel(el)}
                                   onChange={(e) => {
                                       el.selected = e.checked
                                       setTask({
                                           ...task,
                                           mountedFileSystems: mountedFileSystems['MANUAL'].filter(el => el.selected)
                                       })
                                   }}
                                   checked={el.selected} />
                            <Text label='Mount point'
                                  value={el.mountPath}
                                  onChange={(e) => {
                ...
                <div>
                    <div>
                      ...
                      <Table
                        selection={iscsiDisks} onSelectionChange={e => {
                    ...
                    <div>
                      <div>
                        ...
                        <Chips value={task.allowedClients} onChange={(e) => {
                            setTask({
                                ...task,
