...
import { BackupDropdown } from '../../../components/input/BackupDropdown';
...
export class RestoreModal extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      (!prevState.task.backup && this.state.task.backup) ||
      (prevState.task.backup &&
        prevState.task.backup.guid !== this.state.task.backup.guid)
    ) {
      this.backupChange();
    }

    if (
      (!prevState.task.hypervisorManager &&
        this.state.task.hypervisorManager) ||
      (prevState.task.hypervisorManager &&
        prevState.task.hypervisorManager.guid !==
          this.state.task.hypervisorManager.guid)
    ) {
      this.getHypervisorStoragesForHypervisorManager(
        this.state.task.hypervisorManager.guid,
      );
      this.getHypervisorClustersForHypervisorManager(
        this.state.task.hypervisorManager.guid,
      );
    }
    ...
    if (
      ...
      (prevState.storage && prevState.storage.uuid !== this.state.storage.uuid)
    ) {
      this.passStorageToTask();
    }

    if (
      (!prevState.cluster && this.state.cluster) ||
      (prevState.cluster && prevState.cluster.uuid !== this.state.cluster.uuid)
    ) {
  ...
  passStorageToTask() {
    this.setState({
      task: { ...this.state.task, restoreStorageId: this.state.storage.uuid },
    });
  }

  passClusterToTask() {
    this.setState({
      task: { ...this.state.task, restoreClusterId: this.state.cluster.uuid },
    });
  }
  ...
  getBackups() {
    vprotectService
      .getRestorableBackups(this.props.virtualEnvironment.guid)
      .then((result) => {
        this.setState({
          task: { ...this.state.task, backup: result[0] },
          backups: result,
        });
      });
  }
  ...
  getHypervisorManagersAvailableForBackup() {
    vprotectService
      .getHypervisorManagersAvailableForBackup(this.state.task.backup.guid)
      .then((hypervisorManagers) => {
        if (hypervisorManagers.length > 0) {
          this.setState({ hypervisorManagers: hypervisorManagers });
          if (
            this.props.virtualEnvironment.hvManager &&
            this.isHypervisorInArray(
              this.props.virtualEnvironment.hvManager,
              hypervisorManagers,
            )
          ) {
            this.setState({
              task: {
                ...this.state.task,
                hypervisorManager: this.findHypervisorInArray(
                  this.props.virtualEnvironment.hvManager,
                  hypervisorManagers,
                ),
              },
            ...
            this.setState({
              task: {
                ...this.state.task,
                hypervisorManager: hypervisorManagers[0],
              },
            });
          }
  ...
  isHypervisorInArray(hypervisor, hypervisors) {
    return !!hypervisors && hypervisors.length > -1
      ? hypervisors.map((el) => el.guid).indexOf(hypervisor.guid) > -1
      : false;
  }
  ...
  getHypervisorStoragesForHypervisorManager(id) {
    vprotectService.getHypervisorStoragesForHvm(id).then((storages) => {
  ...
  getHypervisorClustersForHypervisorManager(id) {
    vprotectService.getHypervisorClustersForHvm(id).then((clusters) => {
        ...
        <Modal.Body className="form">
          ...
          {this.state.cluster && this.state.cluster.name === 'Other' && (
            <div>
              ...
              <InputText
                value={this.state.task.restoreClusterId}
                onChange={(e) => {
                  this.setState({
                    task: {
                      ...this.state.task,
          ...
          {this.state.storage && this.state.storage.name === 'Other' && (
            <div>
              ...
              <InputText
                value={this.state.task.restoreStorageId}
                onChange={(e) => {
                  this.setState({
                    task: {
                      ...this.state.task,
          ...
          <div>
            ...
            <ToggleButton
              checked={this.state.task.overwrite}
              onChange={(e) => {
                this.setState({
                  task: {
                    ...this.state.task,
          ...
          <div>
            ...
            <ToggleButton
              checked={this.state.specifyRestoredVirtualEnvironmentName}
              onChange={(e) => {
                this.setState({
          ...
          {this.state.specifyRestoredVirtualEnvironmentName && (
            <div>
              ...
              <InputText
                value={this.state.task.restoredPeName}
                onChange={(e) => {
                  this.setState({
                    task: {
                      ...this.state.task,
